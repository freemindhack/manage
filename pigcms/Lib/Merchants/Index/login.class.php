...
//				$user = $this->admini->get_one(array('account'=>$data['username']));
//			}
            $user = $this->merchants->get_one(array('username'=>$data['username']));
            $user=$user?$user:$this->employee->get_one(array('account'=>$data['username']));
			if(!$user){
...
//			}
            $_SESSION['merchant']['mid'] = $user['mid'];
            $_SESSION['merchant']['aid'] = $user['aid'];
